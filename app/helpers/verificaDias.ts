function verificaDias(diaStringAgendamento: string){
    const diaAtual: Array<any> = [new Date().getDay()];
    let liberarAgendamento: boolean = false;
    let diaAgendamento = 0;
    if(diaAtual[0] == 0) diaAtual.push('Domingo');
    if(diaAtual[0] == 1) diaAtual.push('Segunda-Feira');
    if(diaAtual[0] == 2) diaAtual.push('Terça-Feira');
    if(diaAtual[0] == 3) diaAtual.push('Quarta-Feira');
    if(diaAtual[0] == 4) diaAtual.push('Quinta-Feira');
    if(diaAtual[0] == 5) diaAtual.push('Sexta-Feira');
    if(diaAtual[0] == 6) diaAtual.push('Sábado');
